...
export const FIG={
  ...
  pie:(slices)=>{var cx=105,cy=72,r=55,s='';var cs=['#6366f1','#f59e0b','#10b981','#ef4444','#8b5cf6'];var angle=0;slices.forEach((sl,i)=>{var sweep=sl.pct/100*360;var rad1=angle*Math.PI/180,rad2=(angle+sweep)*Math.PI/180;var x1=cx+r*Math.cos(rad1),y1=cy+r*Math.sin(rad1);var x2=cx+r*Math.cos(rad2),y2=cy+r*Math.sin(rad2);var big=sweep>180?1:0;s+='<path d="M'+cx+','+cy+' L'+x1.toFixed(1)+','+y1.toFixed(1)+' A'+r+','+r+' 0 '+big+',1 '+x2.toFixed(1)+','+y2.toFixed(1)+' Z" fill="'+(sl.c||cs[i%5])+'" stroke="#fff" stroke-width="1.5"/>';var mid=(angle+sweep/2)*Math.PI/180;var lx=cx+(r*0.65)*Math.cos(mid),ly=cy+(r*0.65)*Math.sin(mid);s+=SV.t(lx.toFixed(1),ly.toFixed(1),sl.l+' '+sl.pct+'%',{fs:9,c:'#fff',fw:'bold'});angle+=sweep;});return SV.w(s,210,148)}
};
