...
const stripU=s=>s.replace(/cm[²³]?|km\/h|km|mm|m[²³]?|°[cC]?|度|分鐘?|秒|人|本|元|個|公里|公斤|克|毫升|升|小時|\$/g,'');
const parseFrac=s=>{const m=s.match(/^(\d+)[_又](\d+)\/(\d+)$/);if(m)return+m[1]+ +m[2]/+m[3];const f=s.match(/^(\d+)\/(\d+)$/);if(f)return+f[1]/+f[2];if(/^-?\d+(\.\d+)?$/.test(s))return parseFloat(s);return null};
...
export const FIG={
  rect:(w,h,hide)=>{var sc=Math.min(130/w,85/h),rw=w*sc,rh=h*sc,ox=(200-rw)/2,oy=(120-rh)/2+15;var wL=hide==='w'?'?':w+' cm',hL=hide==='h'?'?':h+' cm';return SV.w(SV.re(ox,oy,rw,rh)+SV.ra(ox,oy+rh,'bl')+SV.ra(ox+rw,oy+rh,'br')+SV.ra(ox+rw,oy,'tr')+SV.ra(ox,oy,'tl')+SV.t(ox+rw/2,oy+rh+15,wL)+SV.t(ox-14,oy+rh/2+4,hL,{fs:10}))},
  ...
  pie:(slices)=>{var cx=105,cy=72,r=55,s='';var cs=['#6366f1','#f59e0b','#10b981','#ef4444','#8b5cf6'];var angle=0;slices.forEach((sl,i)=>{var sweep=sl.pct/100*360;if(sweep<=0)return;var col=sl.c||cs[i%5];if(sweep>=359.99){s+=SV.ci(cx,cy,r,{f:col,c:'#fff',sw:1.5})}else{var rad1=angle*Math.PI/180,rad2=(angle+sweep)*Math.PI/180;var x1=cx+r*Math.cos(rad1),y1=cy+r*Math.sin(rad1);var x2=cx+r*Math.cos(rad2),y2=cy+r*Math.sin(rad2);var big=sweep>180?1:0;s+='<path d="M'+cx+','+cy+' L'+x1.toFixed(1)+','+y1.toFixed(1)+' A'+r+','+r+' 0 '+big+',1 '+x2.toFixed(1)+','+y2.toFixed(1)+' Z" fill="'+col+'" stroke="#fff" stroke-width="1.5"/>'}var mid=(angle+sweep/2)*Math.PI/180;var lx=cx+(r*0.65)*Math.cos(mid),ly=cy+(r*0.65)*Math.sin(mid);s+=SV.t(lx.toFixed(1),ly.toFixed(1),sl.l+' '+sl.pct+'%',{fs:9,c:'#fff',fw:'bold'});angle+=sweep});return SV.w(s,210,148)}
};
