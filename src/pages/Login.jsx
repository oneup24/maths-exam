      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading}
            className="w-full py-3 rounded-xl bg-amber-500 text-white font-bold text-sm hover:bg-amber-600 transition disabled:opacity-50">
        ...
        <div className="mt-4 text-center text-sm text-gray-400">
          {mode === 'login' ? (
            <p>No account? <button onClick={() => { setMode('signup'); setError(''); }}
              className="text-amber-600 font-semibold hover:underline">Sign Up</button></p>
          ) : (
            <p>Already have one? <button onClick={() => { setMode('login'); setError(''); }}
              className="text-amber-600 font-semibold hover:underline">Log In</button></p>
          )}
        </div>
